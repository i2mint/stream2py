...
class _SourceBuffer:
    def __init__(self, source_reader_info: dict, stop_event: threading.Event, *, key=None, maxlen: int = 10000):
...
if __name__ == '__main__':
    ...
    sc_reader = sc_buf.mk_reader()  # reader must be made after start to have data related to said start
    time.sleep(2)
    ...
    assert expected_data == sc_reader.last_item[1], f"last_item did not follow last range({start_key}, {stop_key}) call"
    ...
    assert previous_last_item == sc_reader.last_item[1], f"last_item moved but should not when peek=True"
