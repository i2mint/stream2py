...
class FillErrorWithZeroesMixin:
    ...
    def _stream_callback(self, in_data, frame_count, time_info, status_flags):
        ...
        if PaStatusFlags(status_flags) != PaStatusFlags.paNoError:
            ...
            if self._first_error_timestamp is None:
                ...
                self._error_status_flag = PaStatusFlags(status_flags)  # track what errors occured
            else:
                self._error_status_flag |= PaStatusFlags(status_flags)  # use OR to mark any new error status flags
        else:
            if self._first_error_timestamp is not None:    # first ok status after there was an error status
                zeroed_data, zeroed_count = self._fill_time_interval_with_zeroes(self._first_error_timestamp, timestamp)
                self.data.append(self.data_to_append(self._first_error_timestamp,
                                                     zeroed_data,
                                                     zeroed_count,
                                                     time_info,
    ...
    def _fill_time_interval_with_zeroes(self, first_error_status_ts, first_ok_status_ts):
        ...
        single_zero_sample = b'\x00' * self._init_kwargs['channels'] * self._init_kwargs['width']  # interleaved zeros
        samples_per_time_unit = self._init_kwargs['rate'] / self.timestamp_seconds_to_unit_conversion
...
class FillErrorWithOnesMixin:
    ...
    def _stream_callback(self, in_data, frame_count, time_info, status_flags):
        ...
        if PaStatusFlags(status_flags) != PaStatusFlags.paNoError:
            ...
            if self._first_error_timestamp is not None:    # first ok status after there was an error status
                zeroed_data, zeroed_count = self._fill_time_interval_with_ones(self._first_error_timestamp, timestamp)
                self.data.append(self.data_to_append(self._first_error_timestamp,
                                                     zeroed_data,
                                                     zeroed_count,
                                                     time_info,
    ...
    def _fill_time_interval_with_ones(self, first_error_status_ts, first_ok_status_ts, fill_value=1):
        """Create wf bytes of value zero for the entire duration of the error to replace garbled data
        ...
        single_fill_sample = fill_value.to_bytes(self._init_kwargs['width'], 'little') * self._init_kwargs['channels']
        samples_per_time_unit = self._init_kwargs['rate'] / self.timestamp_seconds_to_unit_conversion
...
class RaiseRuntimeErrorOnStatusFlagMixin:
    ...
    def data_to_append(self, timestamp, waveform, frame_count, time_info, status_flags):
        ...
        return super(RaiseRuntimeErrorOnStatusFlagMixin, self).data_to_append(timestamp,
                                                                              waveform,
                                                                              frame_count,
                                                                              time_info,
                                                                              status_flags)
...
class PyAudioSourceReaderRaiseOnError(RaiseRuntimeErrorOnStatusFlagMixin, PyAudioSourceReader):
    """PyAudioSourceReader changed to handle errors by raising RuntimeError"""


class PyAudioSourceReaderWithZeroedErrorsAndDictData(DictDataMixin, FillErrorWithZeroesMixin, PyAudioSourceReader):
    """PyAudioSourceReader changed to handle errors and serve data in an easy to read dict."""


class PyAudioSourceReaderWithOnedErrorsAndDictData(DictDataMixin, FillErrorWithOnesMixin, PyAudioSourceReader):
    """PyAudioSourceReader changed to handle errors and serve data in an easy to read dict."""


def _test_run_PyAudioSourceReader(readerClass=PyAudioSourceReader,
    ...
    source = readerClass(rate=44100, width=2, channels=1, input_device_index=0, frames_per_buffer=4096)
    pprint(source.info)
    try:
        source.open()
        pprint(source.info)
        ...
        while i < 600:
            ...
            if data:
                ...
                print(timestamp, len(in_data), int(status_flags), PaStatusFlags(status_flags))
...
if __name__ == '__main__':
    ...
    _test_run_PyAudioSourceReader(PyAudioSourceReaderWithZeroedErrorsAndDictData,
