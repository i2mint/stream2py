...
class StatusInfo:
    ...
    def disk_free_bytes(path: str = '/') -> dict:
        return {
            "val": int(psutil.disk_usage(path).free),
            "unit": "bytes"
        }
    ...
    def disk_used_percents(path: str = '/') -> dict:
        return {
            "val": float(psutil.disk_usage(path).percent),
            "unit": "%"
        }
    ...
    def network_download_speed() -> dict or None:
        ...
        """
        try:
            with os.popen(StatusInfo.SPEEDTEST_CMD + ' --no-upload --simple ') as speedtest_output:
    ...
    def network_upload_speed() -> dict:
        ...
        """
        try:
            with os.popen(StatusInfo.SPEEDTEST_CMD + ' --no-download --simple ') as speedtest_output:
                for line in speedtest_output:
                    label, value, unit = line.split()
    ...
    @staticmethod
    def all(include_network_download_speed: bool = False,
    ...
    def run(self):
        try:
            while not self._stop.is_set():
                ...
                self._data.append((self._index, self.get_timestamp(),
                                  StatusInfo.all(self.include_network_download_speed,
                                                 self.include_network_upload_speed)))  # (index, timestamp, character)
                self._index += 1
