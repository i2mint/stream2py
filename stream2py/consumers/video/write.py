...
class VideoWriter(BufferReaderConsumer):
    ...
    def start(self):
        self.video_writer = cv2.VideoWriter(
            self.file_name, self._fourcc_int, self.fps, self.frame_size
        )
        super().start()
...
if __name__ == '__main__':
    ...
    with StreamBuffer(source_reader=source_reader, maxlen=1000) as stream_buffer:
        ...
        frame_size = (
            buffer_reader.source_reader_info['frame_width'],
            buffer_reader.source_reader_info['frame_height'],
        )
        fps = buffer_reader.source_reader_info['fps']
        with VideoWriter(
            buffer_reader=buffer_reader,
            interval=0.001,
            file_name='VideoWriter.avi',
            fourcc='MJPG',
            fps=fps,
