.. toctree::
   :maxdepth: 2
   :caption: Contents:

   module_docs/stream2py
   module_docs/stream2py/buffer_consumer
   module_docs/stream2py/buffer_reader
   module_docs/stream2py/examples/indexed_audio
   module_docs/stream2py/examples/source_reader
   module_docs/stream2py/examples/usage/keyboard_and_audio
   module_docs/stream2py/examples/usage/record_audio_to_files
   module_docs/stream2py/examples/usage/triggered_starts
   module_docs/stream2py/examples/usage/video_to_stream_and_file
   module_docs/stream2py/source_reader
   module_docs/stream2py/sources
   module_docs/stream2py/sources/audio
   module_docs/stream2py/sources/keyboard_input
   module_docs/stream2py/sources/pchealth
   module_docs/stream2py/sources/plc
   module_docs/stream2py/sources/raw_plc
   module_docs/stream2py/sources/video
   module_docs/stream2py/stream_buffer
   module_docs/stream2py/utility/getch
   module_docs/stream2py/utility/locked_sorted_deque
   module_docs/stream2py/utility/logger
   module_docs/stream2py/utility/reader_writer_lock
   module_docs/stream2py/utility/sorted_collection
   module_docs/stream2py/utility/sorted_deque
   module_docs/stream2py/utility/typing_hints
